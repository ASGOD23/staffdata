...
Degree Program: ${(record.degree || 'N/A')} ${(record.major || 'N/A')}
...
Publications: ${record.publications || 'N/A'}
  ...
  // Format degree and major into a single string for display
  const formatDegreeProgram = (record) => {
    if (type === 'academic') {
      return `${record.degree || 'N/A'} ${record.major || 'N/A'}`;
    } else {
      return record.qualification || 'N/A';
    }
  };
  ...
  const columns = [
    ...
    {
      title: type === 'academic' ? 'Degree Program' : 'Qualification',
      key: 'degreeProgram',
      render: (_, record) => formatDegreeProgram(record),
    },
    {
      title: 'Year',
      ...
      >
        {currentDetails && (
          <Descriptions bordered column={1}>
            ...
            {type === 'academic' ? (
              <Descriptions.Item label="Degree Program">
                {`${currentDetails.degree || 'N/A'} ${currentDetails.major || 'N/A'}`}
              </Descriptions.Item>
            ) : (
              <>
                <Descriptions.Item label="Qualification">
                  {currentDetails.qualification || 'N/A'}
                </Descriptions.Item>
                <Descriptions.Item label="Field">
                  {currentDetails.field || 'N/A'}
                </Descriptions.Item>
              </>
            )}
